							...
							<c:forEach var="item" items="${categorys}">
								<div class="input-checkbox">
									<label style="font-size: 14px;padding: 0;">
										<span></span>
										<a href='<c:url value="/store/${ item.id }"/>'>
											${item.name}
										</a>
									</label>
								</div>
								...
								<div class="col-md-4 col-xs-6">
									<div class="product">
										...
										<div class="product-body">
											...
											<h4 class="product-price">$${item.price} <del class="product-old-price">$9999</del></h4>
							...
							<ul class="store-pagination">
								<c:forEach var="item" begin="1" end="${ paginateInfo.totalPage }" varStatus="loop">
									<c:if test="${ loop.index == paginateInfo.currentPage }">
										<li class="active">
											<a href='<c:url value="/store/${ idCategory }/${ loop.index }"/>' style="display: block;">${ loop.index }</a>
										</li>
									</c:if>
									<c:if test="${ loop.index != paginateInfo.currentPage }">
										<li>
